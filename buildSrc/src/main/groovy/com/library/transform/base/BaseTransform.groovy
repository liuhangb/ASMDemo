...
abstract class BaseTransform extends Transform {
    ...
    /**
     * key为className,比如: android/support/v4/app/ListFragment$1.class
     */
    protected Map<String, ClassVisitor> mClassVisitorMap = new HashMap<>(10);
    ...
    void processDirectoryInput(Context context, DirectoryInput directoryInput,
        ...
        System.out.println("BaseTransform> src.absolutePath: " + dir.absolutePath)
        System.out.println("BaseTransform> dest.absolutePath: " + dest.absolutePath)
    ...
    void onBeforeTransform() {
        System.out.println("BaseTransform:---------- ${getName()} 开始 ----------")
    };

    void onAfterTransform() {
        System.out.println("BaseTransform: ---------- ${getName()} 结束 ----------")
    };


    void registerClassVisitor(com.library.transform.base.BaseClassVisitor classVisitor) {
        if (classVisitor != null && !mClassVisitorMap.containsKey(classVisitor.getClassName())) {
            mClassVisitorMap.put(classVisitor.getClassName(), classVisitor)
        }
    }
